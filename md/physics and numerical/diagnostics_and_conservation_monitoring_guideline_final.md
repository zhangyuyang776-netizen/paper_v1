...
参考旧项目的经验，`paper_v1` 正式采用如下层级化载体。旧项目中已经证明高价值的载体包括 `run.log`、`scalars.csv`、`interface_diag.csv`、`3D_out/mapping.json + steps/*.npz`、`failed_step_xxxxxx/` 等。  

| 层级            | 载体                                         | 作用                                             |
| ------------- | ------------------------------------------ | ---------------------------------------------- |
| Driver 层      | `run.log`、控制台日志                            | case 配置、每步摘要、失败原因、早停原因                         |
...
* `rank_001_meta.json`
* ...
* `rank_000_snes_last_x.csv`
* `rank_001_snes_last_x.csv`
* ...

旧项目里 `rank_*_meta.json` 与 `rank_*_snes_last_x.csv` 已经证明，对定位“哪个 rank 的哪段自由度先坏掉”非常有效。
